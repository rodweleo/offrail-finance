import { ArrowUpRight, ArrowDownLeft, Phone, Receipt, Wifi } from "lucide-react";

export interface Transaction {
  id: string;
  type: "send" | "receive" | "paybill" | "airtime" | "bundles" | "withdraw" | "deposit";
  description: string;
  amount: string;
  date: string;
...
const TransactionItem = ({ transaction, onClick }: TransactionItemProps) => {
  ...
  const isDebit = ["send", "paybill", "airtime", "bundles", "withdraw"].includes(transaction.type);
  ...
  return (
    ...
    >
      ...
      <div className="text-right">
        <p className={`text-sm font-semibold ${isDebit ? "text-foreground" : "text-primary"}`}>
